...
| ARGO_DIFF_AWS_PREFIX             | N/A                         | no               | `/adrise/argo-diff` | Routed mode only: base path for SSM route maps (`<prefix>/<env>/routes/<repo>/<appproject>`) and Secrets Manager token secrets (`<prefix>/<env>/projects/<appproject>`). |
...
| ARGO_DIFF_ENVS                   | N/A                         | no               | `dev,staging,prod` | Routed mode only: comma-separated hub environments whose route maps are consulted. |
| ARGO_DIFF_REQUIRE_MANIFEST_PATHS | N/A                         | no               | `false`  | Set to `true` to diff **only** applications that carry an `argocd.argoproj.io/manifest-generate-paths` annotation. By default an application without the annotation matches every change to its repo, so in a monorepo where most applications lack it nearly every application is diffed on every pull request and `ARGO_DIFF_TIMEOUT` can expire before the ones that actually changed are reached. An annotation value of `/` or an empty string is an explicit opt-in and still matches everything. Applications skipped for want of the annotation are named in a warning in the PR comment so the missing coverage is visible, but unlike a timeout they do **not** fail the run. |
| ARGO_DIFF_ROUTED                 | N/A                         | no               | `false`  | Set to `true` to enable routed mode (GitHub Actions / run-once only): argo-diff discovers the AppProjects a PR touches from SSM route maps, fetches each project's read-only token from Secrets Manager (aws CLI, ambient credentials), diffs each affected project with its own scoped credentials, and posts a single comment with a per-project summary table. `ARGOCD_SERVER_ADDR`/`ARGOCD_AUTH_TOKEN` are not required. Changed manifest files matching no route are named in a PR warning. Typically combined with `ARGO_DIFF_SPEC_PATHS=true`. |
| ARGO_DIFF_SPEC_PATHS             | N/A                         | no               | `false`  | Set to `true` to match applications to changed files by paths **derived from each application's spec** (source `path`, Helm `valueFiles`/`fileParameters`, `$ref` value files) instead of the `manifest-generate-paths` annotation. Cannot drift like hand-maintained annotations, and fails open: an application whose spec yields no usable patterns (or renders from the repo root) is diffed rather than skipped. When set, `ARGO_DIFF_REQUIRE_MANIFEST_PATHS` and annotations are ignored. |
